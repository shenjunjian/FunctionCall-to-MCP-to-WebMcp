...
import { createStreamProxy, MessageChannelServerTransport } from "@opentiny/next";
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp";
import { CallToolRequestSchema, CallToolResultSchema, ListToolsRequestSchema } from "@modelcontextprotocol/sdk/types";
...
export async function createMcpServerClientPair() {
  ...
  const client = new Client({ name: "web-mcp-client", version: "1.0.0" }, { capabilities: { roots: { listChanged: true } } });
...
export async function createChannelServer(endpoint: string) {
  const server = new McpServer({ name: "web-mcp-server", version: "1.0.0" }, { capabilities: { tools: { listChanged: true } } });
...
function _refreshTools(server: McpServer) {
  ...
  server.server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
    try {
      ...
      const strToolResult = await client.executeTool(request.params.name as string, JSON.stringify(request.params.arguments as any));
      if (!strToolResult) {
      ...
      return {
        content: [{ type: "text", text: error.message || "执行工具失败" }] as any,
        isError: true,
      };
    }
  });
...
export async function connectWebAgent(client: Client, url: string, staticSessionId?: string) {
