...
```html
<Tiny-remoter agent="{agent}">
  <template #header> <div>顶部的标题</div> </template>
  <template #operation> <div>操作按钮,右上功能区</div> </template>
  <template #welcome> <div>没有消息时的内容</div> </template>
  <template #suggestion> <div>建议区,输入框上面</div> </template>
</Tiny-remoter>
```
